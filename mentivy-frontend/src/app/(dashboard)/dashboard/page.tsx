...
export default async function DashboardPage() {
  ...
  try {
    const res = await serverFetch('/guidance/stats');
    stats = res.data;
  } catch (error) {
    console.error('Failed to fetch dashboard stats:', error);
  }

  const completionPercent = Math.min(100, (stats.completedMins / stats.dailyGoalMins) * 100);
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start relative z-10">
            <div>
              ...
              <h3 className="text-2xl font-bold text-slate-900 mt-1">
                {stats.completedMins} / {stats.dailyGoalMins} <span className="text-sm font-normal text-slate-500">mins</span>
              </h3>
            </div>
          ...
          <div className="mt-6">
            ...
            <p className="text-xs font-medium text-primary-600 mt-2">
              {stats.dailyGoalMins - stats.completedMins} mins remaining today
            </p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start relative z-10">
            <div>
              ...
              <h3 className="text-2xl font-bold text-slate-900 mt-1">
                {stats.weakTopics.length} <span className="text-sm font-normal text-slate-500">to revise</span>
              </h3>
            </div>
          ...
          <div className="mt-6 flex flex-col gap-2">
            {stats.weakTopics.length > 0 ? (
              stats.weakTopics.map(topic => (
                <span key={topic} className="text-sm text-slate-700 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100 truncate">
                  {topic}
                </span>
              ))
            ) : (
              <span className="text-sm text-slate-400 italic px-3 py-1.5">No weak topics found yet.</span>
            )}
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <div className="mt-6">
            <p className="text-sm text-slate-600 mb-4">
              {stats.avgAccuracy > 70 ? 'Great progress! You are doing well.' : 'Keep practicing to improve your score.'}
            </p>
