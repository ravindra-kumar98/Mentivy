...
import { Target, TrendingDown, RefreshCw, Loader2, BookOpen, ArrowRight, AlertCircle } from 'lucide-react';
...
export default function StudyPlanPage() {
  const [plan, setPlan] = useState<PlanCategory[]>([]);
  ...
  useEffect(() => {
    const fetchPlan = async () => {
      try {
        const res = await apiClient.get('/guidance/daily-plan');
        setPlan(res.data.data);
      } catch (err: any) {
        setError(err.response?.data?.message || 'Failed to load your study plan.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4">
        ...
        <p className="text-slate-500 font-medium">Building your personalized plan...</p>
      </div>
    );
  }
